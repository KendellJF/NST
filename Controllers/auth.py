...
def create_access_token(identity, expires_minutes=None):
    ...
    # PyJWT v2 returns str, v1 returns bytes - normalize to str
...
def jwt_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if auth and auth.lower().startswith('bearer '):
            ...
            # optional: support token in cookie 'access_token'
        ...
        # Attach identity to flask.g for handlers
...
def verify_credentials(username, password):
    """
    Basic credentials check against app config. Returns True/False.
    If you later store users in DB, replace this to check hashed password.
    """
    admin_user = current_app.config.get('ADMIN_USERNAME')
    admin_pass = current_app.config.get('ADMIN_PASSWORD')
    # If ADMIN_PASSWORD is a hash, you can use check_password_hash(admin_pass, password)
